...
export const DEFAULT_BRAND_SETTINGS: BrandSettings = {
  ...
  footerText: '© 2025 Nashmi Store. All rights reserved.',
  themeColor: 'green',
  customColor: '#10b77f',
...
export default function BrandSettings({ userSettings }: BrandSettingsProps) {
  ...
  const systemSettings = (props as any).systemSettings;
  // Use systemSettings for sub-users, fallback to globalSettings
  const effectiveSettings = systemSettings || currentGlobalSettings;
  const [settings, setSettings] = useState<BrandSettings>(() => getBrandSettings(effectiveSettings || userSettings));
  ...
  useEffect(() => {
    ...
    const brandSettings = getBrandSettings(effectiveSettings || userSettings);
  ...
  const getDisplayUrl = (path: string): string => {
    ...
    // Use the image helper utility for consistent URL generation
