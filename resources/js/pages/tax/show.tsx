...
export default function ShowTax() {
  ...
  const { tax, stats, products, auth, currencySymbol } = usePage().props as any;
  ...
  const handleActionClick = (action: string, permission: string) => {
    ...
    switch (action) {
      case 'edit':
        router.visit(route('tax.edit', tax.id));
        break;
    }
  };
  ...
  const pageActions = [
    ...
    ...(hasPermission('edit-tax') ? [{
      ...
      onClick: () => handleActionClick('edit', 'edit-tax')
    }] : [])
  ];
  ...
  return (
    <PageTemplate 
      ...
      description={t('View detailed information about this tax rule')}
      url="/tax/show"
      actions={pageActions}
      breadcrumbs={[
        { title: t('Dashboard'), href: route('dashboard') },
        { title: t('Tax'), href: route('tax.index') },
        { title: t(`View ${tax.name}`) }
      ]}
    >
      <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{tax.type === 'fixed' ? currencySymbol : ''}{tax.rate}{tax.type === 'percentage' ? '%' : ''}</p>
              </div>
              <div>
        ...
        {hasPermission('manage-products') && products && products.length > 0 && (
          <Card>
            <CardHeader>
