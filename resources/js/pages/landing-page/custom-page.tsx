...
export default function CustomPage() {
  ...
  // SEO Fallbacks from superadmin settings
  ...
  // Extract language-specific content from page content
  // Content is wrapped in <!-- LANG:en --> ... <!-- /LANG:en --> and <!-- LANG:ar --> ... <!-- /LANG:ar -->
  const localizedContent = useMemo(() => {
    const content = page.content || '';
    const currentLang = i18n.language || 'ar';
    
    // Check if content has language markers
    const hasLangMarkers = content.includes('<!-- LANG:en -->') || content.includes('<!-- LANG:ar -->');
    
    if (!hasLangMarkers) {
      // No language markers - return content as-is (backward compatible)
      return content;
    }
    
    // Extract content for current language
    const langRegex = new RegExp(`<!-- LANG:${currentLang} -->([\\s\\S]*?)<!-- /LANG:${currentLang} -->`, 'g');
    ...
    // Fallback: if current language not found, try the other language
    const fallbackLang = currentLang === 'ar' ? 'en' : 'ar';
    const fallbackRegex = new RegExp(`<!-- LANG:${fallbackLang} -->([\\s\\S]*?)<!-- /LANG:${fallbackLang} -->`, 'g');
    const fallbackMatches = [...content.matchAll(fallbackRegex)];
    
    if (fallbackMatches.length > 0) {
      return fallbackMatches.map(m => m[1].trim()).join('\n');
    }
    
    // Ultimate fallback - return original content
    return content;
  }, [page.content, i18n.language]);

  return (
    <>
      <Head>
        <title>{page.meta_title || t(page.title)}</title>
