...
export default function LandingPage() {
  ...
  // Initialize language - Arabic is ALWAYS default unless user explicitly chose English
  React.useEffect(() => {
    const userChoice = localStorage.getItem('userLanguageChoice');
    // Only use English if user explicitly chose it via the toggle button
    const targetLang = (userChoice === 'en') ? 'en' : 'ar';
  ...
  const pageTitle = superadminSettings?.metaTitle || superadminSettings?.titleText || 'Nashmi Store - Build Your Online Store';
  const metaDescription = superadminSettings?.metaDescription || 'Create beautiful, professional online stores with Nashmi Store. Everything you need to start selling online.';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredStores.map((store) => (
              <div key={store.id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow">
                {store.logo && (
                  <img loading="lazy" src={store.logo} alt={store.name} className="w-16 h-16 object-cover rounded-lg mb-4" />
                )}
