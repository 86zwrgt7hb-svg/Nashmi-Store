...
export default function ReferredUsers() {
  ...
  const getFormattedTotalCommission = (user: ReferredUser) => {
    if (!user.referrals || user.referrals.length === 0) return '0';
    const total = getTotalCommission(user);
    return user.referrals[0]?.formatted_amount ? 
      user.referrals.reduce((sum, referral) => {
        const amount = parseFloat(referral.formatted_amount?.replace(/[^0-9.-]/g, '') || '0');
        return sum + amount;
      }, 0).toFixed(2) : 
                            ...
                            return planInfo ? (
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  {(() => {
                                    const latestOrder = user.plan_orders?.[0];
                                    return latestOrder?.formatted_final_price || formatCurrency(planInfo.price);
                                  })()}/{t(planInfo.cycle)}
                                </p>
                              </div>
                            ) : (
                          ...
                          {user.referrals.map((referral) => (
                            <div key={referral.id} className="flex justify-between text-sm">
                              ...
                              <span className="font-medium text-green-600">
                                +{referral.formatted_amount || formatCurrency(referral.amount)}
                              </span>
                            </div>
                          ))}
                        </div>
