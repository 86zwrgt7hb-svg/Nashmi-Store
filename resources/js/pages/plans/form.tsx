...
export default function PlanForm({ plan, hasDefaultPlan = false, otherDefaultPlanExists = false }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    enable_pos: plan?.enable_pos || 'off',
    themes: plan?.themes || [],
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border rounded-lg p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="enable_pos">{t("POS System")}</Label>
                <Switch
                  id="enable_pos"
                  checked={formData.enable_pos === 'on'}
                  onCheckedChange={(checked) => handleSwitchChange('enable_pos', checked)}
                />
              </div>
