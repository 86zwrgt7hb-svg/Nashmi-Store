...
export default function Plans({ plans: initialPlans, billingCycle: initialBillingCycle = 'monthly', hasDefaultPlan, isAdmin = false, currentPlan, userTrialUsed, userOnTrial = false, trialDaysLeft = 0, trialPeriodActive = false, paymentMethods = [] }: Props) {
  const { t } = useTranslation();
  const { auth } = usePage().props as any;
  const [plans, setPlans] = useState<Plan[]>(initialPlans);
  // Use currentPlan's duration if available, otherwise use initialBillingCycle
  const [billingCycle, setBillingCycle] = useState<'monthly' | 'yearly'>(
    currentPlan?.duration === 'yearly' ? 'yearly' : initialBillingCycle
  );
  ...
  const handleSwitchTrialPlan = async (planId: number) => {
    try {
      const csrfToken = document.querySelector('meta[name=csrf-token]')?.getAttribute('content') || '';
      const response = await fetch(route('plans.switchTrial'), {
        method: 'POST',
        headers: {
      ...
      // Full page reload to avoid React hooks mismatch
      window.location.href = route('plans.index');
    } catch (error) {
      toast.error(t('Failed to switch plan'));
    }
  };
  ...
  const handleDowngradeToFree = async () => {
    try {
      const csrfToken = document.querySelector('meta[name=csrf-token]')?.getAttribute('content') || '';
      const response = await fetch(route('plans.downgradeToFree'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': csrfToken,
      ...
      toast.error(t('Failed to switch to free plan'));
    }
  };
  ...
  const getActionButton = (plan: Plan) => {
    ...
    // If this is the Free plan - show simple "Return to Free Plan" button
    if (plan.is_free) {
      return (
        <Button
          ...
          className="w-full"
        >
          {t('Return to Free Plan')}
        </Button>
      );
    }

    // If user is within trial period - allow instant switching to paid plans
    // This works even if user temporarily downgraded to free (trialPeriodActive checks trial_expire_date)
    if ((userOnTrial || trialPeriodActive) && !plan.is_current) {
      return (
        <div className="space-y-2">
          <Button
            onClick={() => handleSwitchTrialPlan(plan.id)}
            disabled={processing}
            variant="outline"
            className="w-full"
          >
    ...
    // If trial is available and user hasn't used trial yet
    ...
    // Default: Subscribe only (no Request Plan)
    return (
      <div className="space-y-2">
        {hasPermission('subscribe-plans', auth) && (
          <Button
                    ...
                    <div className="relative overflow-hidden bg-white rounded-xl border border-gray-200 p-3 group-hover:border-primary/30 transition-colors">
                      ...
                      <div className="relative flex items-center gap-2 mb-1">
                        ...
                        <div className="text-xl font-bold text-blue-700">
                          {plan.stats.stores == 0 || plan.stats.stores == '0' || plan.stats.stores == 'Unlimited' ? t('Unlimited') : plan.stats.stores}
                        </div>
                      </div>
                    ...
                    <div className="relative overflow-hidden bg-white rounded-xl border border-gray-200 p-3 group-hover:border-primary/30 transition-colors">
                      ...
                      <div className="relative flex items-center gap-2 mb-1">
                        ...
                        <div className="text-xl font-bold text-emerald-700">
                          {plan.stats.users_per_store == 0 || plan.stats.users_per_store == '0' || plan.stats.users_per_store == 'Unlimited' ? t('Unlimited') : plan.stats.users_per_store}
                        </div>
                      </div>
                    ...
                    <div className="relative overflow-hidden bg-white rounded-xl border border-gray-200 p-3 group-hover:border-primary/30 transition-colors">
                      ...
                      <div className="relative flex items-center gap-2 mb-1">
                        ...
                        <div className="text-xl font-bold text-orange-700">
                          {plan.stats.products_per_store == 0 || plan.stats.products_per_store == '0' || plan.stats.products_per_store == 'Unlimited' ? t('Unlimited') : plan.stats.products_per_store}
                        </div>
                      </div>
                    ...
                    <div className="relative overflow-hidden bg-white rounded-xl border border-gray-200 p-3 group-hover:border-primary/30 transition-colors">
                      ...
                      <div className="relative flex items-center gap-2 mb-1">
                        ...
                        <div className="text-xl font-bold text-amber-700">
                          {plan.stats.storage == 0 || plan.stats.storage == '0 GB' || plan.stats.storage == '0' || plan.stats.storage == 'Unlimited' ? t('Unlimited') : plan.stats.storage}
                        </div>
                      </div>
