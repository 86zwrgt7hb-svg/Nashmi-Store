...
export function CrudFormModal({
  ...
  useEffect(() => {
    // We only want to reset the form data if:
    // 1. The modal was closed and is now being opened
    // 2. The initialData passed from the parent has changed (e.g., editing a different item)
    // 3. The mode has changed (e.g., from Edit to Create)
    ...
    if (justOpened || dataChanged || modeChanged) {
