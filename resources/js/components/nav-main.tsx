...
export function NavMain({ items = [], position }: { items: NavItem[]; position: 'left' | 'right' }) {
    ...
    return (
        <SidebarGroup className="px-1.5 py-0">
            <SidebarGroupLabel className={`flex w-full text-xs ${position === 'right' ? 'justify-end' : 'justify-start'}`}>Platform</SidebarGroupLabel>
            <SidebarMenu>
                {items.map((item) => (
                    <div key={item.title}>
                        {item.children ? (
                            ...
                            <>
                                <SidebarMenuItem>
                                    {state === "collapsed" ? (
                                        // When collapsed, use dropdown menu with icon only
                                        <DropdownMenu>
                                        ...
                                        // When expanded, use accordion style
                                        <SidebarMenuButton 
                                            isActive={isChildActive(item.children)} 
                                            tooltip={{ children: item.title }}
                                            onClick={() => item.isLocked ? router.visit('/plans') : toggleExpand(item.title)}
                                        >
                                            <div className={`flex items-center gap-2 w-full ${position === 'right' ? 'justify-end text-right' : 'justify-start text-left'}`}>
                                                {position === 'right' ? (
                                                    <>
                                                        {state !== "collapsed" && item.isLocked && <Lock className="h-3 w-3 text-muted-foreground" />}
                                                        <span className={item.isLocked ? 'text-muted-foreground' : ''}>{item.title}</span>
                                                        {item.icon && <item.icon className={`h-4 w-4 ${item.isLocked ? 'text-muted-foreground' : ''}`} />}
                                                        {!item.isLocked && (expandedItems[item.title] ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />)}
                                                    </>
                                                ) : (
                                                    <>
                                                        {item.icon && <item.icon className={`h-4 w-4 ${item.isLocked ? 'text-muted-foreground' : ''}`} />}
                                                        {state !== "collapsed" && <span className={item.isLocked ? 'text-muted-foreground' : ''}>{item.title}</span>}
                                                        {state !== "collapsed" && item.isLocked ? <Lock className="h-3 w-3 text-muted-foreground ml-auto" /> : (expandedItems[item.title] ? <ChevronDown className="h-3 w-3 ml-auto" /> : <ChevronRight className="h-3 w-3 ml-auto" />)}
                                                    </>
                                                )}
                                            </div>
                                        </SidebarMenuButton>
                                ...
                                {/* Child items - only show when expanded and not locked */}
                                {state !== "collapsed" && !item.isLocked && expandedItems[item.title] && renderSubMenu(item.children)}
                            </>
                        ) : (
                                        ...
                                        >
                                            {position === 'right' ? (
                                                ...
                                                <>
                                                    {item.icon && <item.icon className={`h-4 w-4 ${item.isLocked ? 'text-muted-foreground' : ''}`} />}
                                                    {state !== "collapsed" && <span className={item.isLocked ? 'text-muted-foreground' : ''}>{item.title}</span>}
                                                    {state !== "collapsed" && item.isLocked && <Lock className="h-3 w-3 text-muted-foreground ml-auto" />}
                                                </>
                                            )}
                                        </a>
                                    ) : (
                                        <Link
                                            href={item.isLocked ? '/plans' : (item.href || '#')}
                                            prefetch
                                            className={`flex items-center gap-2 ${position === 'right' ? 'justify-end text-right' : 'justify-start text-left'}`}
                                        >
                                            {position === 'right' ? (
