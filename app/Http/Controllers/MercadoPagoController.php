...
class MercadoPagoController extends Controller
{
    ...
    private function getMercadoPagoCredentials()
    {
        ...
        $mode = 'production';
        if ($accessToken && str_starts_with($accessToken, 'TEST-')) {
            $mode = 'sandbox';
        } else {
            $mode = $settings['payment_settings']['mercadopago_mode'] ?? 'production';
        }
    ...
    public function createPreference(Request $request)
    {
        try {
            ...
            try {
                ...
                SDK::setIntegratorId("dev_whatsstore");
            } catch (\Exception $e) {
            ...
            $item = new Item();
            $item->title = "Plan: " . $plan->name . " (" . $billingCycle . ")";
            $item->quantity = 1;
            ...
            try {
                ...
                throw new \Exception(__('Failed to save MercadoPago preference: :message', ['message' => $e->getMessage()]));
            }
    ...
    /**
     * Handle successful store order payment
     * Redirect URL format: /store/{slug}/mercadopago/success/{order_number}
     */
    public function storeSuccess(Request $request, $orderNumber)
    {
        try {
            $paymentId = $request->payment_id;
            ...
            if ($externalReference && $externalReference !== $orderNumber) {
                return redirect()->route('store.checkout.error')->with('error', __('Invalid payment reference'));
            }
            ...
            return redirect()->route('store.checkout.error')
                ->with('error', __('Payment status could not be verified. Please contact support.'));
                
        } catch (\Exception $e) {
            Log::error('MercadoPago store success failed: ' . $e->getMessage());
    ...
    {        
        try {
            ...
            // Determine if this is a plan or order payment based on external_reference format
            // Plan: plan_{plan_id}_{user_id}_{billing_cycle}[_coupon_{code}]
            // Order: {order_number}
            $parts = explode('_', $externalReference);
