...
class CategoryController extends Controller
{
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        foreach ($request->ids as $id) {
            $category = Category::where('store_id', $currentStoreId)->find($id);
            ...
            if ($hasSubcategories) {
                $skippedCount++;
                $errors[] = __('":name" has subcategories and was skipped.', ['name' => $category->name]);
                continue;
            }
            ...
            if (method_exists($category, 'products') && $category->products()->count() > 0) {
                $skippedCount++;
                $errors[] = __('":name" has products and was skipped.', ['name' => $category->name]);
                continue;
            }
        ...
        if ($skippedCount > 0) {
            $message .= ' ' . __(':count categories were skipped because they have subcategories or products.', ['count' => $skippedCount]);
        }
